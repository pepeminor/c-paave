...
export const LinkPreview = React.memo(
  ({
    ...
    }, [enableAnimation, onPreviewDataFetched, previewData, requestTimeout, text]);
        ...
        <>
          <View style={StyleSheet.flatten([styles.textContainer, textContainerStyle])}>
            ...
            {(data?.description ||
              ...
              data?.title) && (
              <View style={StyleSheet.flatten([styles.metadataContainer, metadataContainerStyle])}>
                <View style={StyleSheet.flatten([styles.metadataTextContainer, metadataTextContainerStyle])}>
          ...
          {/* Render image node only if there is an image with an aspect ratio not equal to 1
            ...
            */}
          {data?.image && (aspectRatio !== 1 || (!data?.description && !data.title)) && renderImageNode(data.image)}
        </>
      )({
        aspectRatio,
        containerWidth,
    ...
    const renderMinimizedImageNode = (image: PreviewDataImage) => {
      return oneOf(
        renderMinimizedImage,
        <Image accessibilityRole="image" source={{ uri: image.url }} style={styles.minimizedImage} />
      )(image);
    };
    ...
    return (
      <TouchableWithoutFeedback accessibilityRole="button" onPress={handlePress} {...touchableWithoutFeedbackProps}>
