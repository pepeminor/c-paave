/* tslint:disable */
import { defineAnimation, WithDecayConfig, WithSpringConfig } from 'react-native-reanimated';
export function withDecaySpring(userConfig: WithDecayConfig & WithSpringConfig & { clamp: [number, number] }) {
  'worklet';
    ...
    function decaySpring(animation: any, now: number) {
      ...
      if (Array.isArray(config.clamp)) {
        if (animation.moveBack) {
          ...
          const criticallyDampedPosition = toValue - criticallyDampedEnvelope * (x0 + (springV0 + omega0 * x0) * t);
